...
- **A wedged CI step outlived the job timeout meant to catch it.** `Mobile / Mobile overflow audit (390px)` hung inside `playwright install` and was still reporting `in_progress` 26 minutes in — six minutes past the job's own `timeout-minutes: 20`, because a job timeout is enforced by the runner process and a wedged runner never fires it. The PR read as "CI still running" with every other check green, and an integration token can neither cancel nor re-run a workflow (403). Both steps in `.github/workflows/mobile.yml` now carry their own `timeout-minutes`, which fails the step so the job can report it. The same job took six minutes on the previous PR, so this was a hung runner rather than a slow one.
